...
export default function MovieGrid({
  ...
  if (isLoading) {
    return (
      ...
      >
        <SectionHeader
          seedMovie={seedMovie}
          subtitle="Finding similar movies…"
        />
  ...
  if (error) {
    return (
      <section
        aria-label="Recommendation error"
        className="mt-12 animate-fade-up"
      >
  ...
  // Empty — API returned 200 but recommendations array is empty
  if (movies.length === 0) {
    return (
      <section
        aria-label="No recommendations found"
        className="mt-12 animate-fade-up"
      >
        <SectionHeader seedMovie={seedMovie} />
  ...
  return (
    <section aria-label="Recommendations" className="mt-12 animate-fade-up">
      <SectionHeader
        seedMovie={seedMovie}
        subtitle={`${movies.length} similar movie${movies.length !== 1 ? "s" : ""}`}
      />
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 mt-6">
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3 border-b border-zinc-800 pb-4">
      <h2 className="text-lg sm:text-xl font-bold text-white">
        Because you liked <span className="text-accent">"{seedMovie}"</span>
      </h2>
